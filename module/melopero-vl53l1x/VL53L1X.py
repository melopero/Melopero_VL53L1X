...
class VL53L1X():
    ...
    def set_intermeasurement_period(self, millis):
        ...
        status = self._cfuncs.VL53L1_SetInterMeasurementPeriodMilliSeconds(self._cfuncs.GetDevicePointer(), millis)
        if status != 0:
    ...
    def set_measurement_timing_budget(self, millis):
        ...
        status = self._cfuncs.VL53L1_SetMeasurementTimingBudgetMicroSeconds(self._cfuncs.GetDevicePointer(), millis*1000)
        if status != 0:
            print('something went wrong error : {}'.format(status))
...
if __name__ == '__main__':
    a = VL53L1X()
    a.set_intermeasurement_period(0)
    a.set_measurement_timing_budget(50)
    a.start_ranging(VL53L1X.MEDIUM_DST_MODE)
    while(True):
        print(a.get_measurement())
    a.stop_ranging()
    a.close_connection()
